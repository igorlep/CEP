...
{
  header->Caption="Ðååñòð íà îïëàòó ðåàëèçîâàííîé ãàçåòíî-æóðíàëüíîé ïðîäóêöèè çà "+periodStr;
}
//---------------------------------------------------------------------------
...
{
  periodStr=monthBox->Text+" "+yearEdit->Text+" ãîäà";
...
TDateTime date1=beginOfMonth(d_date);
  ...
  if(dFrame->getWorkDataSet()->RecordCount > 0)
  { if(MessageDlg("Âíèìàíèå! Ïðè çàïîëíåíèè ðååñòðà ñóùåñòâóþùèå äàííûå áóäóò óäàëåíû. Ïðîäîëæèòü?",mtInformation,TMsgDlgButtons() << mbOK << mbCancel,0)==mrCancel)
      return;
  }
...
TAddRecordsToRoolForm *AF;
  ...
  { MessageDlg("Îøèáêà ñîçäàíèÿ ôîðìû TAddRecordsToRoolForm\n"+exc.Message,mtError,TMsgDlgButtons() << mbOK,0);
    goto end;
  }
...
TpFIBDataSet *ds=dFrame->getWorkDataSet();
  ...
  if(grid->SelectedRows->Count==1)
    str="Âû óâåðåíû, ÷òî õîòèòå óäàëèòü òåêóùóþ çàïèñü?";
  else
    str="Âû óâåðåíû, ÷òî õîòèòå óäàëèòü âûáðàííûå çàïèñè?";
...
AnsiString sql;
  ...
  if(MessageDlg("Âû óâåðåíû, ÷òî õîòèòå óäàëèòü ñóùåñòâóþùèå äàííûå ðååñòðà?",mtConfirmation,TMsgDlgButtons() << mbOK << mbCancel,0)==mrCancel)
    return;
...
AnsiString fName=workDir+"Reports\\RoolOnPaymnetsReport.fr3";
  ...
  { MessageDlg("Íå îáíàðóæåí ôàéë ñ øàáëîíîì îò÷åòà - "+fName,mtWarning,TMsgDlgButtons() << mbOK,0);
    return;
  }
  frxXLSExport->FileName="Ðååñòð íà îïëàòó.xls";
  frxDS->DataSet=dFrame->getWorkDataSet();
  frxReport->PrepareReport(true);
  frxReport->ShowPreparedReport();
}
//---------------------------------------------------------------------------
//Ïå÷àòü ðàñïîðÿæåíèÿ íà ïëàòåæ
...
AnsiString fName=workDir+"Reports\\DictationOnPayment.fr3";
  ...
  frxXLSExport->FileName="Ðàñïîðÿæåíèå íà ïëàòåæ.xls";
  frxReport->PrepareReport(true);
  frxReport->ShowPreparedReport();
}
//---------------------------------------------------------------------------
void __fastcall TDictationOnPaymentFrame::frxReportGetValue(const AnsiString VarName, Variant &Value)
{
  //Ðååñòð íà îïëàòó
  if(VarName=="Title")
  { AnsiString s="Ðååñòð íà îïëàòó ðåàëèçîâàííîé ãàçåòíî-æóðíàëüíîé ïðîäóêöèè çà ";
    s+=periodStr;
    Value=s;
  }
  ...
  //Ðàñïîðÿæåíèå íà ïëàòåæ
  else if(VarName=="Payee") //ïîëó÷àòåëü ïëàòåæà
  {
    Value="Ñîãëàñíî ðååñòðà íà îïëàòó ðåàëèçîâàííîé ÃÆÏ çà "+periodStr;
  }
  else if(VarName=="PurposeOfPayment") //íàçíà÷åíèå ïëàòåæà
  {
    Value="Îïëàòà ðåàëèçîâàííîé ãàçåòíî-æóðíàëüíîé ïðîäóêöèè çà "+periodStr;
  }
  else if(VarName=="BaseOfPayment") //îñíîâàíèå ïëàòåæà
  {
    Value="Ñ÷åòà ñîãëàñíî ðååñòðà íà îïëàòó ðåàëèçîâàííîé ÃÆÏ çà "+periodStr;
  }
  else if(VarName=="Summa") //ñóììà ïëàòåæà
  { AnsiString summa="";
    int cn=dFrame->findPozitionColumnInGrid("PAYMENT_S");
    if(cn>0) summa=dFrame->workGrid->SumList->SumCollection->Items[3]->SumValue;
    Value=summa;
  }
  else if(VarName=="SummaPropis") //ñóììà ïðîïèñüþ
  { double summa=0;
    int cn=dFrame->findPozitionColumnInGrid("PAYMENT_S");
    if(cn>0) summa=dFrame->workGrid->SumList->SumCollection->Items[3]->SumValue;
